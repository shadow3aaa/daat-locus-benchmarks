...
class BenchmarkRunnerTests(unittest.TestCase):
    ...
    def test_daat_locus_send_adapter_sends_prompt_on_stdin(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn("# Task: task-send", sent_prompt)
            self.assertIn(f"Repository Path: {task_dir}", sent_prompt)
            self.assertIn("Open this project and fix this problem, do not commit.", sent_prompt)
            self.assertIn("## Problem statement", sent_prompt)
            self.assertIn("fix via send", sent_prompt)
            self.assertNotIn("Benchmark task", sent_prompt)
            self.assertNotIn("Runner context", sent_prompt)
            self.assertNotIn("Run artifact", sent_prompt)
            self.assertNotIn("Patch artifact", sent_prompt)
            self.assertNotIn("benchmark runner", sent_prompt)
